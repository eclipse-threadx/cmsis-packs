...
def get_component_from_cmake_file(cmake_file):
    ...
    while True:
        ...
        subdir_line = contents[pos + len('add_subdirectory(') : end_pos].strip('')
...
def get_file_from_cmake_file(cmake_file, files_element):
    ...
    if 'target_include_directories(' in contents:
        # Extract the source files from the target_include_directories() directive
        start_pos = contents.find('target_include_directories(')
        end_pos = contents.find(')', start_pos)
        include_list = contents[start_pos + len('target_include_directories(') : end_pos]

        # Parse the source files
        incs = include_list.split('$')
        c_incs = [source for source in incs if source.endswith('inc') or source.endswith('include')]
        if len(c_incs) > 0:
            for inc in c_incs:
                output_str = inc.replace("{CMAKE_CURRENT_LIST_DIR}", current_dir)
    ...
    if 'target_sources(' in contents:
        ...
        if len(c_sources) > 0:
            for c_source in c_sources:
                output_str = c_source.replace("{CMAKE_CURRENT_LIST_DIR}", current_dir)
                output_str = output_str[3:]
                output_str = output_str.replace("\\", "/")
                file_element = ET.SubElement(files_element, "file")
...
def update_component(component, bundle_subcomponent, azrtos_module_name, porting_files):
        ...
        description_element.text = "Azure RTOS " + azrtos_module_name + " " + component
        rte_element = ET.SubElement(component_element, "RTE_Components_h")
        if component == "core" or component == "common":
            rte_element.text = "#define AZURE_RTOS_" + azrtos_module_name.upper() + "_ENABLED"
        else:
            rte_element.text = "#define AZURE_RTOS_" + component.upper() + "_ENABLED"
...
def generate_pdsc_file(module_source_path, module_data_path, cmsis_pack_working_path):
    # pdsc template file name
    pdsc_template_file = os.path.join(cmsis_pack_working_path, PDSC_TEMPLATE_FILE_NAME)
    ...
    root.set('xs:noNamespaceSchemaLocation', 'https://raw.githubusercontent.com/Open-CMSIS-Pack/Open-CMSIS-Pack-Spec/v1.7.7/schema/PACK.xsd')

    # get common dir, porting dir, component name and supported porting devices from template
    ...
    # supported device porting files
    # this is a key-value store, key is condition, value is porting file directory
    # such as {"CA5 GNU Condition": "ports/cortex_a5/gnu/"}
    porting_files = {}

    conditions_subcomponent = root.find('conditions')

    # check each ports_device and its compiler variants, then add TCompiler and DCore conditions to pdsc
    for device in port_devices:
        ...
        if os.path.isdir(ports_device_dir) == False:
        ...
        for subdir in os.listdir(ports_device_dir):
            ...
            condition_element = ET.SubElement(conditions_subcomponent, "condition")
            condition_element.attrib['id'] = id
            description_element = ET.SubElement(condition_element, "description")
    ...
    if os.path.isfile(cmake_file) == False:
    ...
    if len(components_list):
        for component in components_list:
            ...
            files_element = update_component(component, bundle_subcomponent, azrtos_module_name, porting_files)
            sub_cmake_file = os.path.join(common_dir, component, 'CMakeLists.txt')
